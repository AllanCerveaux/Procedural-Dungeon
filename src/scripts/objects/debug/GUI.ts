...
function PlayerGUI(pane: Pane) {
  ...
  life_controller
    ...
    .on('click', ({ index }) => {
      const [x, y] = index;
      if (x === 0 && y === 0)
        PlayerEmitter.emit(PLAYER_EMITTER.HEALTH_UP, 'heart');
      if (x === 0 && y === 1)
        PlayerEmitter.emit(PLAYER_EMITTER.HEALTH_DOWN, 'heart');
      if (x === 1 && y === 0)
        PlayerEmitter.emit(PLAYER_EMITTER.HEALTH_UP, 'extra');
      if (x === 1 && y === 1)
        PlayerEmitter.emit(PLAYER_EMITTER.HEALTH_DOWN, 'extra');
    });
  life_controller
    .addBlade({
      view: 'buttongrid',
    ...
    .on('click', ({ index }) => {
      const [x, y] = index;
      if (x === 0 && y === 0)
        PlayerEmitter.emit(PLAYER_EMITTER.HEAL, 1, 'heart');
      if (x === 0 && y === 1)
        PlayerEmitter.emit(PLAYER_EMITTER.DAMAGE, 1, 'heart');
      if (x === 1 && y === 0)
        PlayerEmitter.emit(PLAYER_EMITTER.HEAL, 1, 'extra');
      if (x === 1 && y === 1)
        PlayerEmitter.emit(PLAYER_EMITTER.DAMAGE, 1, 'extra');
    });
