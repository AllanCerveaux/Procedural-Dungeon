...
export default class Player extends Base {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, { key, name }: { key: string; name: string }) {
    ...
    PlayerEmitter.on(PLAYER_EMITTER.PICKUP_ITEM, (item: Item, quantity: number = 1) => this.inventory.add(item, quantity))
    PlayerEmitter.on(PLAYER_EMITTER.DROP_ITEM, (item: Item, quantity: number = 1, isDropped: boolean = false) => this.inventory.remove(item, quantity, isDropped))
    PlayerEmitter.on(STATS_EMITTER.STATS_CHANGE, (state: 'UP' | 'DOWN', name: StatisticsType, cost: number) => this.updateStatistic(state, name, cost))
  }
